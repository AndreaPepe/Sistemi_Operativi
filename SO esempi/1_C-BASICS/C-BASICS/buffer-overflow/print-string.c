...
#define x86_64 (sizeof(void*)==8)	// per vedere se il SO è a 64 bit
...
int main(int arc, char** argv){
 ...
 if (x86_64)
	target_pc = "\x37\x07\x40\x00\x00\x00\x00\x00"; //please keep this from the disassembly of the target program
 else
	target_pc = "\x24\x85\x04\x08"; //please keep this from the disassembly of the target program

 if (argv[1]){
	 magic_number = strtol(argv[1],NULL,10);	// string to long integer
 }
 else{
