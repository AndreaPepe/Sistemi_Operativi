...
retry:
	...
	sem_wait(&(sem_free_slot[my_index]));	// wait until the slot becomes free
	
	v[my_index] = data;

	sem_post(&(sem_available_item[my_index])); // set data as available
...
retry:
	sem_wait(&(sem_available_item[my_index]));	// wait until data become available
	value = v[my_index];
	...
	sem_post(&(sem_free_slot[my_index]));		// data read makes slot free to be rewritten
...
int main(int argc, char** argv){
	...
	for (i=0; i<SIZE; i++){
		if(sem_init(&(sem_free_slot[i]),1,1) == -1){		// init: UNNAMED SEMAPHORES
