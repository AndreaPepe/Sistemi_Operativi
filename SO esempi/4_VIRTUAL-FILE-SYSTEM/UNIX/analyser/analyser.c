...
do {
   ...
   while (res_r){
      ...
      index=0;
		...
		// printa solo se è cambiato qualcosa:
if ((linenum!= temp_linenum) || (wordnum!= temp_wordnum) ||(charnum!= temp_charnum)) { 
   ...
   if (continous) {sleep(1); lseek(fd, 0,SEEK_SET);}  // file pointer ad inizio file con offset 0
} while(continous);		// se chiamato con -p continua fino a interruzione forzata
exit(0);
}
