...
int count_args(char *start_command, int *pipe_present, char **next_command)
{
   ...
   * argomento (se eseiste), ed incrementare quindi il numero
...
int main ()
{
  ...
  while (1)
  {
    ...
    do
    {
       ...
       *
       *  - il numero di argomenti che compondo il comnado nell'attuale
       *    stage della command-pipe;
       *
       *  - l'eventuale presenza di un successivo comando nella command-
       *    pipe;
       *
